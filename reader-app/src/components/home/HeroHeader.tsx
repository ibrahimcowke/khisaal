...
export function HeroHeader({ showBack = false }: { showBack?: boolean }) {
  ...
  return (
    <header className="relative -mx-4 sm:-mx-6 lg:-mx-8 -mt-4 sm:-mt-6 lg:-mt-6 mb-6 sm:mb-8 rounded-b-[2.5rem] bg-linear-to-b from-app-surface via-app-surface/98 to-app-accent/8 border-b border-app-border/80 px-4 sm:px-8 lg:px-10 pt-5 sm:pt-8 pb-5 sm:pb-7 shadow-xs overflow-hidden transition-all duration-300">
      ...
      <div className="relative z-10 space-y-4 sm:space-y-5">
        {/* Row 1: Balanced Top Navigation Bar */}
        <div className="flex items-center justify-between gap-2">
          {/* Start: Time Capsule & Date Badges */}
          <div className="flex items-center gap-1.5 sm:gap-2 min-w-0 flex-wrap">
            {showBack && (
              <button
                ...
                aria-label={t('back')}
                title={t('back')}
                className="h-8 w-8 flex items-center justify-center rounded-xl bg-app-surface border border-app-border text-app-text hover:text-app-accent hover:border-app-accent transition-all active:scale-95 shadow-xs shrink-0"
              >
            ...
            {/* Time Badge */}
            <div className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1 rounded-full border shadow-xs text-xs font-bold shrink-0 ${timeInfo.bg}`}>
              {timeInfo.icon}
              <span className="truncate max-w-36 sm:max-w-none">{timeInfo.label}</span>
            </div>

            {/* Date Pill */}
            <div className="hidden xs:flex items-center gap-1 px-2.5 sm:px-3 py-1 rounded-full bg-app-surface border border-app-border/80 text-xs font-semibold text-app-text-secondary shadow-xs shrink-0">
              <span>{formattedDate}</span>
            </div>
          </div>

          {/* End: Sleek Utility Controls */}
          <div className="flex items-center gap-1 p-1 rounded-2xl bg-app-surface/90 border border-app-border/90 shadow-xs backdrop-blur-md shrink-0">
            {/* Language Switch */}
            <button
              onClick={toggleLang}
              className="flex items-center gap-1 px-2 py-1 sm:px-2.5 sm:py-1.5 rounded-xl hover:bg-app-accent/10 text-app-text hover:text-app-accent transition-all active:scale-95 text-xs font-bold shrink-0"
            ...
            <button
              ...
              className="h-7 w-7 sm:h-8 sm:w-8 flex items-center justify-center rounded-xl hover:bg-app-accent/10 text-app-text-secondary hover:text-app-accent transition-all active:scale-95 shrink-0"
              aria-label={t('library')}
              title={isRtl ? 'المكتبة والكتب' : 'Library & Books'}
            >
              <BookMarked size={15} />
            </button>
            ...
            <button
              ...
              className="h-7 w-7 sm:h-8 sm:w-8 flex items-center justify-center rounded-xl hover:bg-app-accent/10 text-app-text-secondary hover:text-app-accent transition-all active:scale-95 shrink-0"
              aria-label={t('stats')}
              title={isRtl ? 'إحصائيات وإنجاز القراءة' : 'Reading Progress & Stats'}
            >
              <TrendingUp size={15} />
            </button>
        ...
        {/* Row 2: Hero Greeting & Dedicated Primary "Read Now" CTA */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3.5 pt-1">
          ...
          {/* Primary Direct Reading Button (Prominent & Clean on both Mobile and Desktop) */}
          <button
            onClick={handleOpenReader}
            className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-2xl bg-app-accent text-white hover:bg-app-accent/90 transition-all active:scale-95 shadow-md text-xs sm:text-sm font-bold shrink-0 self-stretch sm:self-auto group"
            title={isRtl ? 'فتح قارئ الكتاب مباشرة' : 'Open Reader'}
          >
            <BookOpen size={16} className="group-hover:scale-110 transition-transform" />
            <span>{isRtl ? 'استئناف القراءة' : 'Resume Reading'}</span>
            <span className="text-xs opacity-70">📖</span>
          </button>
        </div>

        {/* Row 3: Quick Search Launch Bar */}
        <div
          onClick={() => navigate('/search')}
          className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl bg-app-surface border border-app-border hover:border-app-accent/60 hover:shadow-md transition-all cursor-pointer shadow-xs group"
        >
          <Search size={16} className="text-app-muted group-hover:text-app-accent transition-colors shrink-0" />
          <span className="text-xs sm:text-sm text-app-muted font-medium truncate flex-1">
            {isRtl ? 'ابحث في نصوص وفصول الموسوعة والحِكَم...' : 'Search encyclopedia, chapters & wisdom...'}
          </span>
        ...
        {/* Row 4: Quick Tools Header Strip */}
